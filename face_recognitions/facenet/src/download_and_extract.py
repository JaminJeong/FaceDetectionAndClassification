...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(prog="Download facenet Models",
                                     description="Download facenet Models", add_help=True)
    parser.add_argument('-m', '--MODELNAME', help='model name.', required=True)
    parser.add_argument('-p', '--DOWNLOADPATH', help='downlaod path', required=True)
    args = parser.parse_args()

    if os.path.isdir(args.DOWNLOADPATH) == False:
        os.mkdir(args.DOWNLOADPATH)

    download_and_extract_file(args.MODELNAME, args.DOWNLOADPATH)